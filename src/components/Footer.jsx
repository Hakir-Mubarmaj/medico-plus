// eslint-disable-next-line no-unused-vars
import React from 'react';

const Footer = () => {
  return (
  <footer className="bg-gradient-to-b from-[#643d90] to-[#eb5d7e] text-white">
  <div className="container mx-auto px-4 text-center">
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
  <div
    className="flex items-center justify-center border-b-2 border-neutral-200 p-6 dark:border-neutral-500 lg:justify-between">
    <div className="mr-12 hidden lg:block">
      <span>Get connected with us on social networks:</span>
    </div>
    <div className="flex justify-center">
      <a href="https://www.facebook.com/medicoplus2012" className="mr-6 text-neutral-100 dark:text-neutral-200">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="h-4 w-5 hover:opacity-25"
          fill="currentColor"
          viewBox="0 0 24 24">
          <path
            d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z" />
        </svg>
      </a>
      <a href="https://l.facebook.com/l.php?u=https%3A%2F%2Fapi.whatsapp.com%2Fsend%3Fphone%3D%252B8801760085833%26context%3DARCHWXVHI165pA7-PZCrUh5Fyg62Sw922bx_WF-jKk9829RhZv-xq6xJ8aKxo_FxQCop8eMIwD-jZJNxS_VbWm69EloBk3i09FoyQy_Spo8m-IaEMrysm0WNiZ7tVsbOMSiQ6RFwZxi5H3nbjJnSn504rA%26source%3DFB_Page%26app%3Dfacebook%26entry_point%3Dpage_cta%26fbclid%3DIwZXh0bgNhZW0CMTAAAR2MBX8G89eUhNpWD_UVqQnPPCJyKhO4JuOcYr-WqV-Iev5PL884FKmD5LU_aem_IpRI-z72N3d9A1cc1DKlLA&h=AT1ukT30ko5ov_wnG9fJJzYmnSP91x6MPals0r8mVSQ8vaX-KLS4Fl0K-dJP3WsUBvLjYSJQMuhV9ByACJMJmWUagYmRf2ouGA73YppZcfhhDGJ7c4_jRdGlIGXe1Icl3_M2gg" className="mr-6 text-neutral-100 dark:text-neutral-200">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="h-4 w-5 hover:opacity-25"
          fill="currentColor"
          viewBox="0 0 24 24">
          <path
            d="M17.472 14.382c-.297-.149-1.758-.867-2.031-.967-.273-.1-.472-.149-.67.149-.198.297-.767.966-.94 1.164-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.473-.883-.788-1.48-1.762-1.655-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.52.149-.173.198-.297.298-.496.099-.198.05-.372-.025-.521-.074-.149-.669-1.612-.916-2.207-.242-.579-.488-.501-.67-.511h-.576c-.173 0-.446.064-.68.297-.198.198-.26.447-.297.644-.035.198-.035.496.124.762.149.297.519.893 1.13 1.458.613.564 1.366 1.013 1.561 1.12.198.1.347.074.471-.124.124-.198.522-.613.661-.825.149-.173.298-.173.496-.099.198.074 1.255.593 1.755.898.261.149.447.223.644.347.297.149.447.124.613-.074.174-.223.774-.897.974-1.195.099-.198.049-.347-.074-.496zM12 0C5.373 0 0 5.373 0 12c0 2.111.553 4.093 1.61 5.845L.047 23.953l6.242-1.625A11.962 11.962 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22.126a10.126 10.126 0 0 1-5.182-1.438l-.372-.223-3.703.964.99-3.607-.248-.372A10.122 10.122 0 1 1 12 22.126z" />
        </svg>
      </a>
    </div>
  </div>
  <div className="mx-6 py-10 text-center md:text-left">
    <div className="grid-1 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
      <div className="mb-4 flex items-left justify-center font-semibold uppercase md:justify-start">
      <img className="h-auto max-w-xs" src="https://i.ibb.co.com/bJb6s1R/logo-demo.png" alt="image description"/>
      </div>
      <div className="padding">
        <h6
          className="mb-4 flex justify-center font-semibold uppercase md:justify-start">
          Useful links
        </h6>
        <p className="mb-4">
          <a href="#" className="text-neutral-200 dark:text-neutral-200 hover:opacity-25">Back To Home</a>
        </p>
        <p className="mb-4">
          <a href="#about" className="text-neutral-200 dark:text-neutral-200 hover:opacity-25">About Us</a>
        </p>
        <p className="mb-4">
          <a href="#projects" className="text-neutral-200 dark:text-neutral-200 hover:opacity-25">Courses</a>
        </p>
        <p>
          <a href="#contact" className="text-neutral-200 dark:text-neutral-200 hover:opacity-25">Contact</a>
        </p>
      </div>
      <div>
        <h6
          className="mb-4 flex justify-center font-semibold uppercase md:justify-start">
          Contact
        </h6>
        <p className="mb-4 flex items-center justify-center md:justify-start">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="currentColor"
            className="mr-3 h-5 w-5">
            <path
              d="M11.47 3.84a.75.75 0 011.06 0l8.69 8.69a.75.75 0 101.06-1.06l-8.689-8.69a2.25 2.25 0 00-3.182 0l-8.69 8.69a.75.75 0 001.061 1.06l8.69-8.69z" />
            <path
              d="M12 5.432l8.159 8.159c.03.03.06.058.091.086v6.198c0 1.035-.84 1.875-1.875 1.875H15a.75.75 0 01-.75-.75v-4.5a.75.75 0 00-.75-.75h-3a.75.75 0 00-.75.75V21a.75.75 0 01-.75.75H5.625a1.875 1.875 0 01-1.875-1.875v-6.198a2.29 2.29 0 00.091-.086L12 5.43z" />
          </svg>
          Orchid building Beside Teletalk Customer Care,
        </p>
        <p className="mb-4 flex items-center justify-center md:justify-start">Bottola, Bogra Road, Barishal, Bangladesh.</p>
        <p className="mb-4 flex items-center justify-center md:justify-start">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="currentColor"
            className="mr-3 h-5 w-5">
            <path
              d="M1.5 8.67v8.58a3 3 0 003 3h15a3 3 0 003-3V8.67l-8.928 5.493a3 3 0 01-3.144 0L1.5 8.67z" />
            <path
              d="M22.5 6.908V6.75a3 3 0 00-3-3h-15a3 3 0 00-3 3v.158l9.714 5.978a1.5 1.5 0 001.572 0L22.5 6.908z" />
          </svg>
          medicoplus103@gmail.com
        </p>
        <p className="mb-4 flex items-center justify-center md:justify-start">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="currentColor"
            className="mr-3 h-5 w-5">
            <path
              fillRule="evenodd"
              d="M1.5 4.5a3 3 0 013-3h1.372c.86 0 1.61.586 1.819 1.42l1.105 4.423a1.875 1.875 0 01-.694 1.955l-1.293.97c-.135.101-.164.249-.126.352a11.285 11.285 0 006.697 6.697c.103.038.25.009.352-.126l.97-1.293a1.875 1.875 0 011.955-.694l4.423 1.105c.834.209 1.42.959 1.42 1.82V19.5a3 3 0 01-3 3h-2.25C8.552 22.5 1.5 15.448 1.5 6.75V4.5z"
              clipRule="evenodd" />
          </svg>
          01787-457944
        </p>
      </div>
    </div>
  </div>
  <hr className="h-px my-0 bg-gray-200 border-10 dark:bg-white"></hr>
  <div className="grid-1 grid gap-8 md:grid-cols-2 lg:grid-cols-2">
  <div className=" p-6 text-left w-full">
    <span>© 2024 Copyright:</span>
    <a> Medico Plus LTD</a>
  </div>
  <div className=" p-6 text-right w-full">
    <span>Developed By:</span>
    <a> The Chiefs</a>
  </div>
  </div>
</div>
</div>
</footer>
  );
};

export default Footer;
